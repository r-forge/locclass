...
test_that("FLXMCLqda: mlr interface works", {
	...
	# centers, method
	lrn <- makeLearner("classif.FLXMCLqda", centers = 3)
	tr1 <- train(lrn, task)
	...
	set.seed(120)
	lrn <- makeLearner("classif.FLXMCLqda", par.vals = list(centers = 3, iter.max = 200), predict.type = "prob")
	tr2 <- train(lrn, task)
	...
	tr3 <- myStepFlexmix(y ~ ., data = as.data.frame(data), model = FLXMCLqda(), cluster = cluster, control = list(iter.max = 200, tolerance = 10^-2))
	tr3 <- flexmix(y ~ ., data = as.data.frame(data), model = FLXMCLqda(), cluster = posterior(tr3), control = list(iter.max = 200))
