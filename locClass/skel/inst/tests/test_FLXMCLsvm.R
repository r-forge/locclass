...
test_that("FLXMCLsvm: training data from only one class", {
	...
	expect_that(fit <- flexmix(Species ~ Sepal.Width + Sepal.Length, data = iris[1:50,], concomitant = FLXPwlda(~ Sepal.Width + Sepal.Length), model = FLXMCLsvm(kernel = "linear", fitted = FALSE), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("need training data from at least two classes"))
})
...
test_that("FLXMCLsvm: missing classes in individual clusters", {
	set.seed(123)
	cluster <- kmeans(iris[,1:4], centers = 2)$cluster
	expect_that(fit <- flexmix(Species ~ Sepal.Width, data = iris, concomitant = FLXPwlda(~ Sepal.Length), model = FLXMCLsvm(kernel = "linear", fitted = FALSE), cluster = cluster, control = list(iter.max = 200, classify = "hard")), gives_warning("some groups are empty"))
...
test_that("FLXMCLsvm: removing clusters works", {
	...
	tr2 <- flexmix(y ~ ., data = as.data.frame(data), concomitant = FLXPwlda(~ x.1 + x.2), model = FLXMCLsvm(), cluster = cluster, control = list(iter.max = 100))
...
test_that("predict FLXMCLsvm works correctly with missing newdata", {
	...
	tr2 <- flexmix(y ~ ., data = as.data.frame(data)[ran,], concomitant = FLXPwlda(~ x.1 + x.2), model = FLXMCLsvm(kernel = "linear", fitted = FALSE), cluster = cluster, control = list(iter.max = 200))
...
test_that("predict FLXMCLsvm works with missing classes in the training data", {
	...
	expect_that(tr2 <- flexmix(Species ~ ., data = iris[1:100,], concomitant = FLXPwlda(~ Sepal.Width + Sepal.Length), model = FLXMCLsvm(), cluster = cluster, control = list(iter.max = 200)), gives_warning("some groups are empty"))
...
test_that("predict FLXMCLsvm works with one single predictor variable", {
	...
	tr2 <- flexmix(Species ~ Sepal.Width, data = iris[ran,], concomitant = FLXPwlda(~ Sepal.Width), model = FLXMCLsvm(kernel = "linear", fitted = FALSE), cluster = cluster, control = list(iter.max = 200))
...
test_that("predict FLXMCLsvm: NA handling in newdata works", {
	ran <- sample(1:150,100)
	cluster <- kmeans(iris[ran,1:4], centers = 2)$cluster
	tr2 <- flexmix(Species ~ Sepal.Length + Sepal.Width, data = iris[ran,], concomitant = FLXPwlda(~ Sepal.Width + Petal.Width), model = FLXMCLsvm(kernel = "linear", fitted = FALSE), cluster = cluster, control = list(iter.max = 200))
	...
	expect_equal(all(is.na(pred[[1]][1:17,])), TRUE)
	expect_equal(all(is.na(pred[[2]][1:17,])), TRUE)
	expect_warning(pred <- mypredict(tr2, newdata = irisna, aggregate = TRUE))
	expect_equal(all(is.na(pred[[1]][1:17,])), TRUE)
})
...
test_that("predict FLXMCLsvm: misspecified arguments", {
	ran <- sample(1:150,100)
	cluster <- kmeans(iris[ran,1:4], centers = 2)$cluster
	tr2 <- flexmix(Species ~ Sepal.Width + Petal.Width, data = iris[ran,], concomitant = FLXPwlda(~ Sepal.Width + Petal.Width), model = FLXMCLsvm(), cluster = cluster, control = list(iter.max = 200))
    # errors in newdata
