...
test_that("generatewf: if there are ties more than k values are larger than zero", {
	z <- 3
	k <- 50
	x <- (0:100)/1000
	x <- c(x, rep(x[k],z), x[100])

	expect_equal(sum(biweight(k = k)(x) > 0), k+z)
	expect_equal(sum(cauchy(k = k)(x) > 0), k+z)
	expect_equal(sum(cosine(k = k)(x) > 0), k+z)
	expect_equal(sum(epanechnikov(k = k)(x) > 0), k+z)
	expect_equal(sum(exponential(k = k)(x) > 0), k+z)
	expect_equal(sum(gaussian(k = k)(x) > 0), k+z)
	expect_equal(sum(optcosine(k = k)(x) > 0), k+z)
	expect_equal(sum(rectangular(k = k)(x) > 0), k+z)
	expect_equal(sum(triangular(k = k)(x) > 0), k+z)

	expect_equal(sum(locClass:::generatewf("biweight", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("cauchy", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("cosine", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("epanechnikov", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("exponential", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("gaussian", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("optcosine", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("rectangular", k = k, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("triangular", k = k, n = 101)(x) > 0), k+z)

	expect_equal(sum(biweight(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(cauchy(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(cosine(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(epanechnikov(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(exponential(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(gaussian(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(optcosine(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(rectangular(k = k, bw = 5)(x) > 0), k+z)
	expect_equal(sum(triangular(k = k, bw = 5)(x) > 0), k+z)

	expect_equal(sum(locClass:::generatewf("biweight", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("cauchy", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("cosine", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("epanechnikov", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("exponential", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("gaussian", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("optcosine", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("rectangular", k = k, bw = 5, n = 101)(x) > 0), k+z)
	expect_equal(sum(locClass:::generatewf("triangular", k = k, bw = 5, n = 101)(x) > 0), k+z)
})
