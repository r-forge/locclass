...
test_that("wlda works", {
	...
	# default na.omit
	expect_warning(wlda(Species ~ ., data = irisna, subset = 6:60),"group virginica is empty or weights in this group are all zero")
	# na.fail
	...
	#fit1 <- wlda(Species ~ ., data = irisna, weights = 1:150, subset = 6:60, na.action = na.omit)

	# one predictor variable
	d <- wlda(Species ~ Petal.Width, data = iris, subset = 6:60)
	...
	#expect_error(wlda(Species ~ Petal.Width, data = iris, subset = 1))   ## funktioniert

	# check if weighted solution with weights = 1 equals weighted batch solution
	l2 <- wlda(Species ~ ., data = iris, weights = rep(1,150))
	l1 <- wlda(Species ~ ., data = iris)
	...
	# l1 <- wlda(Species ~ ., data = iris, weights = c(rep(0.8,110), rep(1,40)))
	...
	# l1 <- wlda(Species ~ ., data = iris[-(111:150),], weights = c(rep(0.8,60), rep(1,50)))
	# checkEquals(l1[c(1,3,5)],l2[c(1,3,5)])                                                  ### ???
