...
test_that("FLXMCLmultinom: misspecified arguments", {
	...
	expect_that(fit <- flexmix(Species ~ V1, data = iris, concomitant = FLXPmultinom(~ Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("Objekt 'V1' nicht gefunden"))
	expect_that(fit <- flexmix(Species ~ Sepal.Length, data = iris, concomitant = FLXPmultinom(~ V1), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("Objekt 'V1' nicht gefunden"))
	expect_that(fit <- flexmix(y ~ Sepal.Length, data = iris, concomitant = FLXPmultinom(~ Sepal.Width), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("Objekt 'y' nicht gefunden"))
	# wrong class
	expect_error(fit <- flexmix(iris, data = iris, concomitant = FLXPmultinom(~ Sepal.Width), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")))
...
test_that("FLXMCLmultinom without concomitant variable model",{
	...
	## weighted
	fit <- flexmix(Species ~ Sepal.Width, data = iris, model = FLXMCLmultinom(decay = 0.1), cluster = cluster, control = list(iter.max = 200, classify = "weighted", verb = 1))
...
test_that("FLXMCLmultinom throws a warning if grouping variable is numeric", {
	cluster <- kmeans(iris[,1:4], centers = 3)$cluster
	expect_that(tr2 <- flexmix(Petal.Width ~ Petal.Length + Sepal.Length, data = iris, concomitant = FLXPmultinom(~ Petal.Length + Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), gives_warning("'Y' was coerced to a factor"))
})
...
test_that("FLXMCLmultinom ist set up correctly", {
	...
	fit <- flexmix(Species ~ Sepal.Width, data = iris[1:100,], concomitant = FLXPmultinom(~ Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard"))
	library(nnet)	
	...
	set.seed(120)
	cluster <- kmeans(iris[,1], centers = 2)$cluster
	fit <- flexmix(Species ~ Sepal.Width, data = iris, concomitant = FLXPmultinom(~ Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard"))
...
test_that("FLXMCLmultinom: training data from only one class", {
	...
	expect_that(fit <- flexmix(Species ~ Sepal.Width + Sepal.Length, data = iris[1:50,], concomitant = FLXPmultinom(~ Sepal.Width + Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), throws_error("need two or more classes to fit a multinom model"))
})
...
test_that("FLXMCLmultinom: missing classes in individual clusters", {
	...
	expect_that(fit <- flexmix(Type ~ ., data = Glass, concomitant = FLXPmultinom(as.formula(paste("~", paste(colnames(Glass)[1:9], collapse = "+")))), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200, classify = "hard")), gives_warning("groups ‘1’ ‘3’ are empty"))
...
test_that("predict FLXMCLmultinom works with missing classes in the training data", {
	...
	expect_that(tr2 <- flexmix(Species ~ ., data = iris[1:100,], concomitant = FLXPmultinom(~ Sepal.Width + Sepal.Length), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200)), gives_warning("group ‘virginica’ is empty")) ## warning
...
test_that("predict FLXMCLmultinom works with one single predictor variable", {
	...
	tr2 <- flexmix(Species ~ Sepal.Width, data = iris[ran,], concomitant = FLXPmultinom(~ Sepal.Width), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200))
...
test_that("predict FLXMCLmultinom: NA handling in newdata works", {
	ran <- sample(1:150,100)
	cluster <- kmeans(iris[ran,1:4], centers = 2)$cluster
	tr2 <- flexmix(Species ~ Sepal.Length + Sepal.Width, data = iris[ran,], concomitant = FLXPmultinom(~ Sepal.Width + Petal.Width), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200))
...
test_that("predict FLXMCLmultinom: misspecified arguments", {
	ran <- sample(1:150,100)
	cluster <- kmeans(iris[ran,1:4], centers = 2)$cluster
	tr2 <- flexmix(Species ~ Sepal.Width + Petal.Width, data = iris[ran,], concomitant = FLXPmultinom(~ Sepal.Width + Petal.Width), model = FLXMCLmultinom(), cluster = cluster, control = list(iter.max = 200))
    # errors in newdata
