...
#' (see \code{\link[=generatewf]{wfs}}) and pass them to \code{daqda}. 
#'
...
#'  \item{bw}{(Only if \code{wf} is a string or was generated by means of one of the functions documented in \code{\link[=generatewf]{wfs}}.) 
#'	  The bandwidth used, \code{NULL} if \code{bw} was not specified.}
#'	\item{k}{(Only if \code{wf} is a string or was generated by means of one of the functions documented in \code{\link[=generatewf]{wfs}}.) 
#'	  The number of nearest neighbors used, \code{NULL} if \code{k} was not specified.}
#'  \item{nn.only}{(Logical. Only if \code{wf} is a string or was generated by means of one of the functions documented in \code{\link[=generatewf]{wfs}} and if \code{k} was
...
#'
#' @seealso \code{\link{predict.daqda}}, \code{\link{wqda}} for a weighted version of Linear Discriminant Analysis and \code{\link{llr}} for discriminant adaptive logistic regression.
#'
#' @examples
...
#' @rdname daqda
#' @method daqda formula
#'
#' @S3method daqda formula

daqda.formula <- function(formula, data, ..., subset, na.action) {
...
#' @rdname daqda
#' @method daqda data.frame
#'
#' @S3method daqda data.frame
...
#' @method daqda matrix
#'
#' @S3method daqda matrix

daqda.matrix <- function (x, grouping, ..., subset, na.action = na.fail) {
    if (!missing(subset)) {
...
#' @rdname daqda
#' @method daqda default
#'
#' @S3method daqda default
...
daqda.default <- function(x, grouping, wf = c("biweight", "cauchy", "cosine", "epanechnikov", 
	...
	daqda.fit <- function(x, grouping, wf, itr, weights = rep(1, nrow(x)), ...) {
		...
		for(i in seq_len(itr)) {
			...
			res <- wqda.default(x = x, grouping = grouping, weights = w[[i+1]], ...)
		}
		names(w) <- 0:itr
		res$weights <- w
		return(res)
...
#    }   
	res <- daqda.fit(x = x, grouping = grouping, wf = wf, itr = itr, ...)
...
#' Fitting models where it matters, \emph{The American Statistician}, \bold{57(2)} 124--130.
#'
#' @seealso \code{\link{daqda}}, \code{\link{wqda}}, \code{\link{llr}}.
#'
#' @examples
#' ## comparison with qda:
#' library(MASS)
...
#' @keywords classif multivariate
#' 
#' @method predict daqda
#' @rdname predict.daqda
#'
#' @S3method predict daqda
