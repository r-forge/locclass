...
setMethod(
		f = "initialize",
		signature = signature("classif.damultinom"),
		def = function(.Object) {
		...
		def = function(.learner, .task, .subset,  ...) {
			f = getFormula(.task)
      		if (.task@desc@has.weights)
        		damultinom(f, data = getData(.task, .subset), weights = .task@weights[.subset], ...)
      		else
        		damultinom(f, data = getData(.task, .subset), ...)
		}
)
