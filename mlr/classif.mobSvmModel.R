...
setMethod(
		f = "initialize",
		signature = signature("classif.mobSvmModel"),
		def = function(.Object) {
		...
		def = function(.learner, .task, .subset,  ...) {
    		...
    		mf[[1]] = as.name("mob_control")
      		if (.task@desc@has.weights)
        		mob(f, data = getData(.task, .subset), weights = .task@weights[.subset], probability = .learner@predict.type == "prob", model = svmModel, control = eval(mf), ...)
      		else  
        		mob(f, data = getData(.task, .subset), probability = .learner@predict.type == "prob", model = svmModel, control = eval(mf), ...)
		}
)
