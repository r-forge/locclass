...
setMethod(
		f = "initialize",
		signature = signature("classif.mobMultinomModel"),
		def = function(.Object) {
...
setMethod(
		f = "trainLearner",
		signature = signature(
				.learner = "classif.mobMultinomModel", 
				.task = "ClassifTask",
				.subset = "integer" 
		),
		...
		def = function(.learner, .task, .subset,  ...) {
    		...
    		mf[[1]] = as.name("mob_control")
      		if (.task@desc@has.weights)
        		mob(f, data = getData(.task, .subset), weights = .task@weights[.subset], model = multinomModel, control = eval(mf), ...)
		   	else  
        		mob(f, data = getData(.task, .subset), model = multinomModel, control = eval(mf), ...)			
		}
)
