...
setMethod(
		f = "initialize",
		signature = signature("classif.mobLdaModel"),
		def = function(.Object) {
		...
		def = function(.learner, .task, .subset,  ...) {
    		...
    		mf[[1]] = as.name("mob_control")
      		if (.task@desc@has.weights)
        		mob(f, data = getData(.task, .subset), weights = .task@weights[.subset], model = ldaModel, control = eval(mf), ...)
      		else  
        		mob(f, data = getData(.task, .subset), model = ldaModel, control = eval(mf), ...)
		}
)   
